...
const getProjects = () => {
  fetch('/api/v1/projects').then(response => response.json()).then(projects => {
...
const getProjectPalettes = projects => {
  projects.forEach(project => {
    fetch(`/api/v1/projects/${project.id}/palettes`).then(response => response.json()).then(palettes => {
...
const addPalette = () => {
  ...
  }).then(response => response.json()).then(parsedData => buildProjectPalette(parsedData)).catch(error => console.log(error));
...
const deletePalette = event => {
  ...
  fetch(`/api/v1/palettes/${paletteId}`, {
    method: 'DELETE'
  }).then(() => $(`.${paletteId}`).remove()).catch(error => console.log(error));
